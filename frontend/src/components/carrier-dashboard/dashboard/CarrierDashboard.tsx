...
export function CarrierDashboard() {
  ...
  // Reset selected when leaving details
  useEffect(() => {
    if (!["job-details", "upload-before", "upload-after", "report-incident"].includes(currentView)) {
  ...
  const renderView = () => {
    switch (currentView) {
      case "overview":
        return <DashboardOverview onViewOrders={() => setCurrentView("orders")} onSelectJob={handleViewJob} />;
      case "orders":
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="flex">
        <DashboardSidebar currentView={currentView} onViewChange={setCurrentView} isOpen={sidebarOpen} onClose={() => setSidebarOpen(false)} />
