...
import { Eye, Search, Flag } from "lucide-react";
import { getCurrentUserId } from "../lib/auth";
import OrderDetailModal from "../components/seller/OrderDetailModal";
import RatingModal from "../components/modal/RatingModal";
import ReportIncidentModal from "../components/modal/ReportIncidentModal";
...
const [selectedOrderForReport, setSelectedOrderForReport] = useState(null);
      ...
      <div className="overflow-x-auto rounded-lg border border-gray-200 bg-white shadow-sm">
        <table className="min-w-full divide-y divide-gray-200 text-sm">
          <thead className="bg-gray-50">
            <tr>
              <th className="px-4 py-3 text-left text-gray-500 uppercase text-xs font-medium">
                Mã đơn
              </th>
              <th className="px-4 py-3 text-left text-gray-500 uppercase text-xs font-medium">
                Địa chỉ lấy
              ...
              <th className="px-4 py-3 text-gray-500 uppercase text-xs font-medium">
                Gói
              </th>
              <th className="px-4 py-3 text-gray-500 uppercase text-xs font-medium">
                Tổng tiền
                  ...
                  <tr key={order._id} className="hover:bg-gray-50 transition">
                    ...
                    <td className="px-4 py-3 text-center">
                      {fb ? (
                        ...
                        <button
                          onClick={() => {
                            setSelectedOrderForRating(order._id);
  ...
  <button
    onClick={() => {
      setSelectedOrderForReport(order._id);
  ...
  >
    <Flag className="w-5 h-5 mx-auto" />
      ...
      {/* ⭐ Modal đánh giá */}
      {isRatingOpen && selectedOrderForRating && (
        <RatingModal
          orderId={selectedOrderForRating}
          customerId={currentUserId}
          onClose={() => {
            setIsRatingOpen(false);
            fetchOrders(); // load lại feedback sau khi gửi
          }}
        />
      )}
